...
class DINOv2Wrapper(VisionTransformerWrapper):
    def __init__(self, model_name, device, smaller_edge_size=224, half_precision=False, feat_layer=3):
    ...
    def load_model(self):
        ...
        self.transform = transforms.Compose([
            transforms.Resize(self.smaller_edge_size, interpolation=transforms.InterpolationMode.BICUBIC, antialias=True),
            transforms.ToTensor(),
    ...
    def extract_features(self, image_tensor):
        with torch.inference_mode():
            ...
            tokens = self.model.get_intermediate_layers(image_batch, n=depth)[self.feat_layer].squeeze()
...
def get_model(model_name, device="cpu", smaller_edge_size=448, **kwargs):
    print(f"Loading model: {model_name}")
    print(f"Device: {device}")
    if model_name.startswith("vit"):
        return ViTWrapper(model_name, device, smaller_edge_size, **kwargs)
    if model_name.startswith("dinov2"):
        return DINOv2Wrapper(model_name, device, smaller_edge_size, **kwargs)
